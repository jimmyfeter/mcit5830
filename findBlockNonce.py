...
def mine_block(k, prev_hash, rand_lines):
    ...
    target_suffix = '0' * k  # Binary string of k trailing zeros
    combined_data = prev_hash + b''.join(line.encode('utf-8') for line in rand_lines)

    nonce = 0
    while True:
        nonce_bytes = str(nonce).encode('utf-8')
        hash_result = hashlib.sha256(combined_data + nonce_bytes).hexdigest()
        
        # Check if the binary representation of the hash has k trailing zeros
        if bin(int(hash_result, 16)).endswith(target_suffix):
            nonce = nonce_bytes  # Found a valid nonce
            break
